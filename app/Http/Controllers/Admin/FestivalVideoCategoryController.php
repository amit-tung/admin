...
use App\Models\FestivalImageCategory;
use App\Models\FestivalVideo;
use App\Models\FestivalVideoCategory;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\Session;
use Yajra\DataTables\DataTables;

class FestivalVideoCategoryController extends Controller
{
    private $data = array(
        'route' => 'admin.festival_video_category.',
        'title' => 'Festival Festivalvideo Category',
        'menu' => 'festival',
        'submenu' => 'festival_video_category',
    );
    ...
    public function index(Request $request) {
        if($request->has('export') && ($request->get('export')=='xlsx' || $request->get('export')=='csv')){
            $categories = FestivalVideoCategory::select(['name','status','created_at','updated_at'])->orderBy('name','ASC')->get()->toArray();
            $this->exportFile($request->get('export'), 'festival_video_category', ['Name', 'Status', 'Created At', 'Updated At'], $categories);
        }

        if ($request->ajax()) {
            $records = FestivalVideoCategory::select('*')->orderBy('id','ASC');
        ...
        return view('admin.festival_video_category.index', $this->data);

    }

    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category add']), 403);
        return view('admin.festival_video_category.create', $this->data);
    }
    ...
    public function store(Request $request) {
        ...
        $inputs['image'] = $this->uploadFile($request, null, 'image', 'festival_video_category');

        $record = new FestivalVideoCategory($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = FestivalVideoCategory::findOrFail($id);
        return view('admin.festival_video_category.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category edit']), 403);
        $record = FestivalVideoCategory::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
      ...
      //  if($inputs['image']){
            $inputs['image'] = $this->uploadFile($request, $record, 'image', 'festival_video_category');
