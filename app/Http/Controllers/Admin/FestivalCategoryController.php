...
{
    private $data = array(
        'route' => 'admin.festival-category.',
        'title' => 'Festival Category',
        'menu' => 'festival',
        'submenu' => 'festival-category',
    );
    ...
    public function index(Request $request) {
        if($request->has('export') && ($request->get('export')=='xlsx' || $request->get('export')=='csv')){
            $categories = FestivalCategory::select(['name','status','created_at','updated_at'])->orderBy('name','ASC')->get()->toArray();
            $this->exportFile($request->get('export'), 'festival_category', ['Name', 'Status', 'Created At', 'Updated At'], $categories);
        }


        if ($request->ajax()) {
            $records = FestivalCategory::with('apps')->orderBy('display_order','ASC');
    ...
    public function create() {
        ...
        return view('admin.festival_category.create', $this->data);
    }
    ...
    public function store(Request $request) {
        ...
        $record = FestivalCategory::create($input);
    ...
    public function show($id) {
        $this->data['record'] = FestivalCategory::findOrFail($id);
        return view('admin.festival_category.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        ...
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_category edit']), 403);
        $record = FestivalCategory::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
