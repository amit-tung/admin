...
class GeneralImageController extends Controller
{
    //
    private $data = array(
        'route' => 'admin.general_image.',
        'title' => 'General Image ',
        'menu' => 'general',
        'submenu' => 'general_image',
    );
    ...
    private function _validate($request, $id = null) {
        $this->validate($request, [
            ...
            'general_image_category_id'=>'required',
...
//        }
        ...
        if ($request->ajax()) {
            $records = GeneralImage::select('*')->with('general_image_category')->orderBy('id','ASC');
        ...
        return view('admin.general_image.index', $this->data);

    }
    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['category add']), 403);
        return view('admin.general_image.create', $this->data);
    }
    public function store(Request $request) {
        ...
        $inputs['image'] = $this->uploadFile($request, null, 'image', 'general_image');

        $record = new GeneralImage($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = GeneralImage::findOrFail($id);
        return view('admin.general_image.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['category edit']), 403);
        $record = GeneralImage::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
       ...
       // $this->_validate($request);
        ...
        $inputs = $request->all();
        $inputs['image'] = $this->uploadFile($request, $record, 'image', 'general_image');
