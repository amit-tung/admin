...
class DharmaVideoController extends Controller
{
    //
    private $data = array(
        'route' => 'admin.dharma_video.',
        'title' => 'Dharma Video',
        'menu' => 'dharma',
        'submenu' => 'dharma_video',
    );
    ...
    private function _validate($request, $id = null) {
        $this->validate($request, [
            ...
            'dharma_video_category_id'=>'required',
            'video'=> 'required',
...
//        }
        ...
        if ($request->ajax()) {
            ...
            $records = DharmaVideo::select('*')->with('dharma_video_category')->orderBy('id','ASC');
        ...
        return view('admin.dharma_video.index', $this->data);

    }
    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category add']), 403);
        return view('admin.dharma_video.create', $this->data);
    }
    public function store(Request $request) {
        ...
        $inputs['video'] = $this->uploadFile($request, null, 'video', 'dharma_video');

        $record = new DharmaVideo($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = DharmaVideo::findOrFail($id);
        return view('admin.dharma_video.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category edit']), 403);
        $record = DharmaVideo::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
      ...
      //$this->_validate($request);
        ...
        $inputs = $request->all();
        $inputs['video'] = $this->uploadFile($request, $record, 'video', 'dharma_video');
    ...
    public function destroy($id) {
        ...
        $record = DharmaVideo::findOrFail($id);
        $this->deleteFile($record,'business_video');
        $record->delete();
