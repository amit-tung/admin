...
{
    private $data = array(
        'route' => 'admin.greeting_category.',
        'title' => 'Greeting FestivalImageCategory',
        'menu' => 'greeting',
        'submenu' => 'greeting_category',
    );
    ...
    public function index(Request $request) {
        ...
        if ($request->ajax()) {
            $records = GreetingCategory::select('*')->orderBy('id','ASC');
        ...
        return view('admin.greeting_category.index', $this->data);

    }

    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category add']), 403);
        return view('admin.greeting_category.create', $this->data);
    }
    ...
    public function store(Request $request) {
        ...
        $inputs['image'] = $this->uploadFile($request, null, 'image', 'greeting_category');

        $record = new GreetingCategory($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = GreetingCategory::findOrFail($id);
        return view('admin.greeting_category.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category edit']), 403);
        $record = GreetingCategory::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
      ...
      //  $this->_validate($request);
        ...
        $inputs = $request->all();
        $inputs['image'] = $this->uploadFile($request, $record, 'image', 'greeting_category');
