...
class DharmaImageCategoryController extends Controller
{
    private $data = array(
        'route' => 'admin.dharma_image_category.',
        'title' => 'Dharma Image Category',
        'menu' => 'dharma',
        'submenu' => 'dharma_image_category',
    );
    ...
    public function index(Request $request) {
        ...
        if ($request->ajax()) {
            $records = DharmaImageCategory::select('*')->orderBy('name','ASC');
        ...
        return view('admin.business_image_category.index', $this->data);

    }

    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category add']), 403);
        return view('admin.dharma_image_category.create', $this->data);
    }
    ...
    public function store(Request $request) {
        ...
        $inputs['image'] = $this->uploadFile($request, null, 'image', 'dharma_image_category');

        $record = new DharmaImageCategory($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = DharmaImageCategory::findOrFail($id);
        return view('admin.dharma_image_category.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category edit']), 403);
        $record = DharmaImageCategory::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
      ...
      //  $this->_validate($request);
        ...
        $inputs = $request->all();
        $inputs['image'] = $this->uploadFile($request, $record, 'image', 'dharma_image_category');
