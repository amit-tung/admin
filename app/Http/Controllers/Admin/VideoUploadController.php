...
//        }
        ...
        if ($request->ajax()) {
            $records = VideoUpload::select('*')->with('videocategory')->orderBy('id','ASC');
        ...
        return view('admin.video_upload.index', $this->data);

    }
    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category add']), 403);
        return view('admin.video_upload.create', $this->data);
    }
    public function store(Request $request) {
        ...
        $inputs['video'] = $this->uploadFile($request, null, 'video', 'category_video');

        $record = new VideoUpload($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = VideoUpload::findOrFail($id);
        return view('admin.video_upload.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category edit']), 403);
        $record = VideoUpload::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
        ...
        $inputs = $request->all();
        $inputs['video'] = $this->uploadFile($request, $record, 'video', 'category_video');
