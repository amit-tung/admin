...
class GeneralImageCategoryController extends Controller
{
    private $data = array(
        'route' => 'admin.general_image_category.',
        'title' => 'General Image Category',
        'menu' => 'general',
        'submenu' => 'general_image_category',
    );
    ...
    public function index(Request $request) {
        ...
        if ($request->ajax()) {
            $records = GeneralImageCategory::select('*')->orderBy('name','ASC');
        ...
        return view('admin.general_image_category.index', $this->data);

    }

    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['category add']), 403);
        return view('admin.general_image_category.create', $this->data);
    }
    ...
    public function store(Request $request) {
        ...
        $inputs['image'] = $this->uploadFile($request, null, 'image', 'general_image_category');

        $record = new GeneralImageCategory($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = GeneralImageCategory::findOrFail($id);
        return view('admin.general_image_category.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['category edit']), 403);
        $record = GeneralImageCategory::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
      ...
      //  $this->_validate($request);
        ...
        $inputs = $request->all();
        $inputs['image'] = $this->uploadFile($request, $record, 'image', 'general_image_category');
