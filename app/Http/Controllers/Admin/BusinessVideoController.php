...
class BusinessVideoController extends Controller
{
    //
    private $data = array(
        'route' => 'admin.business_video.',
        'title' => 'Business Video',
        'menu' => 'business_category',
        'submenu' => 'business_video',
    );
    ...
    private function _validate($request, $id = null) {
        $this->validate($request, [
            ...
            'business_video_category_id'=>'required',
            'video'=> 'required',
...
//        }
        ...
        if ($request->ajax()) {
            $records = BusinessVideo::select('*')->with('business_video_category')->orderBy('id','ASC');
        ...
        return view('admin.business_video.index', $this->data);

    }
    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category add']), 403);
        return view('admin.business_video.create', $this->data);
    }
    public function store(Request $request) {
        ...
        $inputs['video'] = $this->uploadFile($request, null, 'video', 'business_video');

        $record = new BusinessVideo($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = BusinessVideo::findOrFail($id);
        return view('admin.business_video.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category edit']), 403);
        $record = BusinessVideo::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
      ...
      //$this->_validate($request);
        ...
        $inputs = $request->all();
        $inputs['video'] = $this->uploadFile($request, $record, 'video', 'business_video');
