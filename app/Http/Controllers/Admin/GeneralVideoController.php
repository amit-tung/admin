...
class GeneralVideoController extends Controller
{
    //
    private $data = array(
        'route' => 'admin.general_video.',
        'title' => 'General Video ',
        'menu' => 'general',
        'submenu' => 'general_video',
    );
    ...
    private function _validate($request, $id = null) {
        $this->validate($request, [
            ...
            'general_video_category_id'=>'required',
            'video'=> 'required',
...
//        }
        ...
        if ($request->ajax()) {
            $records = GeneralVideo::select('*')->with('general_video_category')->orderBy('id','ASC');
        ...
        return view('admin.general_video.index', $this->data);

    }
    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['category add']), 403);
        return view('admin.general_video.create', $this->data);
    }
    public function store(Request $request) {
        ...
        $inputs['video'] = $this->uploadFile($request, null, 'video', 'general_video');

        $record = new GeneralVideo($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = GeneralVideo::findOrFail($id);
        return view('admin.general_video.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['category edit']), 403);
        $record = GeneralVideo::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
      ...
      //$this->_validate($request);
        ...
        $inputs = $request->all();
        $inputs['video'] = $this->uploadFile($request, $record, 'video', 'general_video');
