...
class BusinessVideoCategoryController extends Controller
{
    private $data = array(
        'route' => 'admin.business_video_category.',
        'title' => 'Business Festival Video Category',
        'menu' => 'business_category',
        'submenu' => 'business_video_category',
    );
    ...
    public function index(Request $request) {
        ...
        if ($request->ajax()) {
            $records = BusinessVideoCategory::select('*')->orderBy('name','ASC');
        ...
        return view('admin.business_video_category.index', $this->data);

    }

    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category add']), 403);
        return view('admin.business_video_category.create', $this->data);
    }
    ...
    public function store(Request $request) {
        ...
        $inputs['image'] = $this->uploadFile($request, null, 'image', 'business_video_category');

        $record = new BusinessVideoCategory($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = BusinessVideoCategory::findOrFail($id);
        return view('admin.business_video_category.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        abort_if(env('APP_ENV')=="demo",403);
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category edit']), 403);
        $record = BusinessVideoCategory::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
      ...
      //  $this->_validate($request);
        ...
        $inputs = $request->all();
        $inputs['image'] = $this->uploadFile($request, $record, 'image', 'business_video_category');
