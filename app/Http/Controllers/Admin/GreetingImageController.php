...
class GreetingImageController extends Controller
{
    //
    private $data = array(
        'route' => 'admin.greeting_image.',
        'title' => 'Greeting Image ',
        'menu' => 'greeting',
        'submenu' => 'greeting_image',
    );
...
//        }
        ...
        if ($request->ajax()) {
            $records = GreetingImage::select('*')->with('greeting_category')->orderBy('id','ASC');
        ...
        return view('admin.greeting_image.index', $this->data);

    }
    public function create() {
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category add']), 403);
        return view('admin.greeting_image.create', $this->data);
    }
    public function store(Request $request) {
        ...
        $inputs['image'] = $this->uploadFile($request, null, 'image', 'greeting_image');

        $record = new GreetingImage($inputs);
        $record->save();
    ...
    public function show($id) {
        $this->data['record'] = GreetingImage::findOrFail($id);
        return view('admin.greeting_image.show', $this->data);
    }
    ...
    public function update(Request $request, $id) {
        ...
        //abort_unless(Auth::guard('admin')->user()->hasAnyPermission(['festival_image_category edit']), 403);
        $record = GreetingImage::findOrFail($id);
        /* Change Status Block */
        if ($request->ajax()) {
       ...
       // $this->_validate($request);
        ...
        $inputs = $request->all();
        $inputs['image'] = $this->uploadFile($request, $record, 'image', 'greeting_image');
